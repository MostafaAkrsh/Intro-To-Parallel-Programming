							...
							const float* const filter, const int filterWidth)
{
	...
	if (thread_2D_pos.x  >= numCols - 1 || thread_2D_pos.y == 0 || thread_2D_pos.x == 0 || thread_2D_pos.y + 1 >= numRows - 1) return;

	int thread_1d_pos = thread_2D_pos.y * numCols + thread_2D_pos.x;



			float temp1 = (4.0 / 9.0) * (inputChannel[thread_1d_pos-1]);
			float temp2 = (6.0 / 9.0) * (inputChannel[thread_1d_pos]);
			float temp3 = (6.0 / 9.0) * (inputChannel[thread_1d_pos+1]);
			float temp4 = (9.0 / 9.0) * (inputChannel[thread_1d_pos- numCols]);
			float temp5 = (1.0 / 9.0) * (inputChannel[thread_1d_pos+ numCols]);
			float temp6 = (5.0 / 9.0) * (inputChannel[thread_1d_pos-1- numCols]);
			float temp7 = (3.0 / 9.0) * (inputChannel[thread_1d_pos+1- numCols]);
			float temp8 = (1.0 / 9.0) * (inputChannel[thread_1d_pos-1+ numCols]);
			float temp9 = (3.0 / 9.0) * (inputChannel[thread_1d_pos+1+ numCols]);

			float output = temp1 + temp2 + temp3 + temp4 + temp5 + temp6 + temp7 + temp8 + temp9;
	...
	const int filterWidth)
{
	...
	gaussian_blur_noshared << <gridSize, blockSize, shared_size >> > (d_red, d_redBlurred, numRows, numCols, d_filter, filterWidth);
	gaussian_blur_noshared << <gridSize, blockSize, shared_size >> > (d_green, d_greenBlurred, numRows, numCols, d_filter, filterWidth);
	gaussian_blur_noshared << <gridSize, blockSize, shared_size >> > (d_blue, d_blueBlurred, numRows, numCols, d_filter, filterWidth);
